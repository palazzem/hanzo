...
3. Hardware/host detection is computed in `playbook.yml` `pre_tasks` (`host_has_systemd` from a stat on `/run/systemd/system`; `model_is_gz302` from `ansible_facts['product_name']` AND `host_has_systemd`) and consumed via `when:` on role tasks (e.g. `roles/hardware/tasks/main.yml:4` gates on `model_is_gz302`). Roles consume these facts; they never redefine them.
